...
from ui.style import (
    ...
    create_pie_chart, display_pie_chart, create_styled_radio_buttons,
    apply_styled_dropdown_css, create_styled_line_chart, create_styled_bar_chart
)

# Generate more realistic performance trend data
def generate_realistic_trends(start, end, periods):
    return [round(x + random.uniform(-0.2, 0.2), 2) for x in np.linspace(start, end, periods)]
...
all_performance_ratings = [
    {"department": "Sales", "exceptional": 12, "exceedsExpectations": 23, "meetsExpectations": 45, "needsImprovement": 15, "unsatisfactory": 5},
    {"department": "Marketing", "exceptional": 8, "exceedsExpectations": 17, "meetsExpectations": 55, "needsImprovement": 18, "unsatisfactory": 2},
    {"department": "Engineering", "exceptional": 15, "exceedsExpectations": 25, "meetsExpectations": 48, "needsImprovement": 10, "unsatisfactory": 2},
    {"department": "Customer Support", "exceptional": 10, "exceedsExpectations": 20, "meetsExpectations": 52, "needsImprovement": 13, "unsatisfactory": 5},
    {"department": "HR", "exceptional": 7, "exceedsExpectations": 18, "meetsExpectations": 60, "needsImprovement": 12, "unsatisfactory": 3},
]
...
def director_performance_dashboard():
    ...
    df_performance_vs_training = pd.DataFrame(generate_performance_vs_training_data(departments))

    # Calculate total employees and average performance per department
    df_performance_ratings['total_employees'] = df_performance_ratings.iloc[:, 1:].sum(axis=1)
    ...
    if selected_department == "All":
        ...
        filtered_performance_ratings = df_performance_ratings[df_performance_ratings["department"] == selected_department]
        filtered_goals = df_goals[df_goals["department"] == selected_department]
        filtered_performers = df_performers[df_performers["department"] == selected_department]
        filtered_performance_vs_training = df_performance_vs_training[df_performance_vs_training["department"] == selected_department]

    # Use styled tabs
    tabs = create_styled_tabs(["Performance Overview", "Goal Achievement", "Employee Performance", "Training Impact"])
...
def performance_overview_tab(filtered_performance_ratings, performance_trends):
    ...
    worst_dept = filtered_performance_ratings.loc[filtered_performance_ratings['avg_performance'].idxmin()]
    ...
    ratings_data = filtered_performance_ratings.melt(
        ...
        value_vars=["exceptional", "exceedsExpectations", "meetsExpectations", "needsImprovement", "unsatisfactory"],
        var_name="rating",
        value_name="count"
    )
    ratings_data['percentage'] = ratings_data['count'] / ratings_data['total_employees'] * 100
    ...
    ratings_data['rating_value'] = ratings_data['rating'].map(lambda x: rating_map[x][0])
    ratings_data['rating_label'] = ratings_data['rating'].map(lambda x: rating_map[x][1])
    ...
    for department in performance_trends.columns[1:]:
        create_styled_line_chart(performance_trends[department], "Month", f"{department} Performance Rating")
...
def goal_achievement_tab(filtered_goals):
    ...
    for _, row in filtered_goals.iterrows():
        ...
        with col3:
            create_styled_metric("Achieved", f"{row['achieved']}%", f"{row['achieved'] - row['total']}% from target")

    # Add a summary chart
    create_styled_bar_chart(filtered_goals['department'], filtered_goals['achieved'], "Department", "Achievement (%)")
...
def employee_performance_tab(filtered_performers):
    ...
    with col1:
        ...
        list = [f"{row['name']} ({row['department']}): {row['rating']}" for _, row in top_performers.iterrows()]
        for item in list:
            st.write(f"• {item}")
    ...
    with col2:
        ...
        list = [f"{row['name']} ({row['department']}): {row['rating']}" for _, row in bottom_performers.iterrows()]
        for item in list:
            st.write(f"• {item}")
...
def training_impact_tab(filtered_performance_vs_training):
    ...
    fig_training = px.scatter(
        ...
        labels={"trainingHours": "Training Hours", "avgPerformance": "Average Performance", "employees": "Number of Employees"}
    )
    ...
    # Overall correlation
    overall_correlation = filtered_performance_vs_training['trainingHours'].corr(filtered_performance_vs_training['avgPerformance'])
    ...
    with col1:
        ...
        st.write("A correlation close to 1 indicates a strong positive relationship between training hours and performance.")

    with col2:
        max_performance = filtered_performance_vs_training.loc[filtered_performance_vs_training['avgPerformance'].idxmax()]
        create_styled_metric("Highest Avg Performance", f"{max_performance['avgPerformance']:.2f}", f"{max_performance['department']}")

    with col3:
        max_training = filtered_performance_vs_training.loc[filtered_performance_vs_training['trainingHours'].idxmax()]
        create_styled_metric("Most Training Hours", f"{max_training['trainingHours']}", f"{max_training['department']}")
    ...
    summary_stats = filtered_performance_vs_training.groupby('department').agg({
        'trainingHours': ['mean', 'min', 'max'],
        'avgPerformance': ['mean', 'min', 'max'],
        'employees': 'sum'
    }).reset_index()
    summary_stats.columns = ['Department', 'Avg Training Hours', 'Min Training Hours', 'Max Training Hours',
                             'Avg Performance', 'Min Performance', 'Max Performance', 'Total Employees']
