...
from ui.style import (apply_styled_dropdown_css, create_multi_bar_chart,
                      create_pie_chart, create_styled_bullet_list,
...
def director_engagement_compliance_dashboard():
    ...
    with tabs[0]:
        ...
        fig = px.line(
            engagement_trends,
            x="quarter",
            y=["overall", "productDev", "customerSupport", "sales", "marketing", "hr"],
        )
    ...
    with tabs[1]:
        ...
        st.write(
            "This chart shows the current compliance rates for different categories:"
        )
        compliance_list = [
            f"{row['category']}: {row['compliance']}% compliant (Change: {row['compliance'] - row['previousCompliance']:+.1f}%)"
        ...
        if min(compliance_data["compliance"]) < 95:
            st.warning(
                "Note: Categories with compliance rates below 95% may need immediate attention."
            )
