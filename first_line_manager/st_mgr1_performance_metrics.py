...
from ui.style import create_styled_bar_chart, apply_styled_dropdown_css, create_styled_tabs, create_styled_line_chart
...
def get_pr_code_review_issues_tickets(duration: str) -> pd.DataFrame:
    ...
    data = {
        ...
        "Code Reviews Completed": np.random.randint(10, 50, len(employees)) * multiplier,
...
def get_average_resolution_time(duration: str) -> Dict[str, float]:
    employees = get_dummy_employees()
    multiplier = 1 if duration == "Monthly" else (0.8 if duration == "Quarterly" else 0.6)
    return {emp: np.random.uniform(1, 5) * multiplier for emp in employees}

def get_sprint_velocity(duration: str) -> pd.DataFrame:
    sprints = range(1, 11) if duration == "Monthly" else (range(1, 5) if duration == "Quarterly" else range(1, 13))
...
def get_bug_fix_rate(duration: str) -> Dict[str, float]:
    employees = get_dummy_employees()
    return {emp: min(1.0, np.random.uniform(0.5, 1.0) * (1.2 if duration != "Monthly" else 1)) for emp in employees}
...
def manager_performance_dashboard():
    ...
    with tabs[1]:
        ...
        with col2:
            ...
            if selected_employee != "All":
                resolution_time_data = {selected_employee: resolution_time_data[selected_employee]}
            create_styled_bar_chart(
        ...
        if selected_employee != "All":
            pr_review_data = pr_review_data[pr_review_data['Employee'] == selected_employee]
    ...
    with tabs[2]:
        ...
        if selected_employee != "All":
            page_metrics_data = page_metrics_data[page_metrics_data['Employee'] == selected_employee]
        
        col1, col2 = st.columns(2)
        
        with col1:
            fig_pages = px.bar(page_metrics_data, x='Employee', y='Pages Created', 
                               title=f"Pages Created per Employee ({duration})")
            st.plotly_chart(fig_pages, use_container_width=True)
        
        with col2:
            fig_edits = px.bar(page_metrics_data, x='Employee', y='Edits and Updates', 
