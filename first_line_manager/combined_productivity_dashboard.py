...
from ui.style import (
    create_styled_metric, create_styled_bullet_list, create_pie_chart, display_pie_chart, create_styled_tabs,
    create_progress_bar, create_styled_line_chart, create_styled_bar_chart, apply_styled_dropdown_css
)
...
def aggregate_employee_data(num_employees):
    total_productivity_score = sum(generate_productivity_score() for _ in range(num_employees))
    total_tasks, total_completed, total_in_progress, total_on_track, total_overdue = [sum(x) for x in zip(*[generate_task_data() for _ in range(num_employees)])]

    aggregated_comm_data = {key: sum(generate_communication_data()[key] for _ in range(num_employees)) for key in generate_communication_data().keys()}
    aggregated_comm_data["avg_email_response_time"] /= num_employees

    aggregated_knowledge_data = {key: sum(generate_knowledge_data()[key] for _ in range(num_employees)) for key in generate_knowledge_data().keys()}

    aggregated_meeting_data = {key: sum(generate_meeting_data()[key] for _ in range(num_employees)) for key in generate_meeting_data().keys()}
    aggregated_meeting_data["avg_duration"] /= num_employees
    aggregated_meeting_data["effectiveness"] = round(aggregated_meeting_data["effectiveness"] / num_employees)
    aggregated_meeting_data["weekly_time_percentage"] = round(aggregated_meeting_data["weekly_time_percentage"] / num_employees)

    aggregated_learning_data = {
        "courses_completed": list(set([course for _ in range(num_employees) for course in generate_learning_data()["courses_completed"]])),
        "certifications": list(set([cert for _ in range(num_employees) for cert in generate_learning_data()["certifications"]])),
        "learning_hours": sum(generate_learning_data()["learning_hours"] for _ in range(num_employees)),
        "conferences_attended": sum(generate_learning_data()["conferences_attended"] for _ in range(num_employees)),
        "skill_improvement": round(sum(generate_learning_data()["skill_improvement"] for _ in range(num_employees)) / num_employees, 1)
    }

    aggregated_code_data = {
        key: sum(generate_code_data()[key] for _ in range(num_employees)) if key != "bugs_fixed" else
        {severity: sum(generate_code_data()["bugs_fixed"][severity] for _ in range(num_employees)) for severity in generate_code_data()["bugs_fixed"].keys()}
        for key in generate_code_data().keys()
    }
    aggregated_code_data["quality_score"] = round(aggregated_code_data["quality_score"] / num_employees, 1)
    aggregated_code_data["bug_fix_rate"] = round(aggregated_code_data["bug_fix_rate"] / num_employees, 1)
    ...
    return {
        ...
        "weekly_completion": [sum(x) for x in zip(*[generate_weekly_task_completion() for _ in range(num_employees)])],
        "communication_data": aggregated_comm_data,
        "email_trend": [round(sum(generate_email_response_trend()[i] for _ in range(num_employees)) / num_employees, 1) for i in range(12)],
...
def manager_productivity_dashboard():
    ...
    col1, col2 = st.columns(2)
    with col1:
        duration = st.selectbox("Select Duration", ["Month", "Quarter", "Half Year", "Year"], key="manager_duration")
    with col2:
        team = st.selectbox("Select Team", ["All Teams"] + df['Team'].unique().tolist(), key="manager_team")
    ...
    meeting_df = agg_df[['Team', 'Meeting Effectiveness', 'Average Meeting Duration', 'Percentage Time in Meetings',
                         'Action Items per Meeting', 'Resolutions per Meeting']]
