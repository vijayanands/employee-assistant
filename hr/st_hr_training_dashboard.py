...
import numpy as np
from ui.style import create_styled_metric, create_styled_tabs, apply_styled_dropdown_css
...
def create_dummy_data():
    ...
    for dept in departments:
        for period in time_periods:
            ...
            data.append({"department": dept, "time_period": period, "completionRate": completion_rate})
...
def hr_training_dashboard():
    ...
    filtered_data = training_completion_data[training_completion_data['time_period'] == duration_option]
    ...
    with tabs[0]:
        ...
        with col1:
            create_styled_metric("Average Learning Hours per Employee", "24.5 hours", "📚")

        with col2:
            create_styled_metric("Training Effectiveness", f"{avg_completion_rate:.1f}%", "📈")
    ...
    with tabs[1]:
        ...
        if filter_option == "All Departments":
            chart_data = filtered_data.groupby('department')['completionRate'].mean().reset_index()
        else:
